...
class Environment {
     ...
     */
    provideEnvsAsCommandInPkg(pkg, suggestions, cmdDuplicationDetector) {
        const configuration = vscode.workspace.getConfiguration('latex-workshop');
    ...
    getEnvFromNode(node, lines) {
        ...
        if (latex_utensils_1.latexParser.isEnvironment(node)) {
            const env = new command_1.CmdEnvSuggestion(`${node.name}`, '', { name: node.name, args: '' }, vscode.CompletionItemKind.Module);
    ...
    getEnvFromContent(content) {
        ...
        while (true) {
            ...
            const env = new command_1.CmdEnvSuggestion(`${result[1]}`, '', { name: result[1], args: '' }, vscode.CompletionItemKind.Module);
    ...
    entryEnvToCompletion(itemKey, item, type) {
        ...
        const suggestion = new command_1.CmdEnvSuggestion(item.name, 'latex', (0, command_1.splitSignatureString)(itemKey), vscode.CompletionItemKind.Module);
