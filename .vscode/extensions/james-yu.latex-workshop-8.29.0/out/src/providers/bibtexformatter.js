...
class BibtexFormatter {
    ...
    async formatDocument(document, sort, align, range) {
        // Get configuration
        const bibtexUtils = new bibtexutils_1.BibtexUtils(this.extension, document.uri);
            ...
            if (latex_utensils_1.bibtexParser.isEntry(entries[i])) {
                ...
                if (isDuplicate && handleDuplicates !== 'Ignore Duplicates') {
                    if (handleDuplicates === 'Highlight Duplicates') {
                        ...
                        this.extension.logger.addLogMessage(`BibTeX-format: Duplicate entry "${entry.internalKey}" at line ${entryLocations[i].start.line + lineDelta + 1 + lineOffset}.`);
                        text = text.replace(/@/, '');
                    }
                }
            }
