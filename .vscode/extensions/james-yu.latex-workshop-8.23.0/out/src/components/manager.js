...
const pdfwatcher_1 = require("./managerlib/pdfwatcher");
const bibwatcher_1 = require("./managerlib/bibwatcher");
const finderutils_1 = require("./managerlib/finderutils");
const pathutils_1 = require("./managerlib/pathutils");
const intellisensewatcher_1 = require("./managerlib/intellisensewatcher");
...
class Manager {
    constructor(extension) {
         ...
         */
        this.cachedContent = Object.create(null);
        this.localRootFiles = Object.create(null);
        this.rootFilesLanguageIds = Object.create(null);
        // Store one root file for each workspace.
        this.rootFiles = Object.create(null);
        this.workspaceRootDirUri = '';
        this.filesWatched = new Set();
        this.fileWatcherMutex = new await_semaphore_1.Mutex();
        this.rsweaveExt = ['.rnw', '.Rnw', '.rtex', '.Rtex', '.snw', '.Snw'];
        this.jlweaveExt = ['.jnw', '.jtexw'];
        this.weaveExt = [];
        ...
        this.pdfWatcher = new pdfwatcher_1.PdfWatcher(extension);
        this.bibWatcher = new bibwatcher_1.BibWatcher(extension);
        this.intellisenseWatcher = new intellisensewatcher_1.IntellisenseWatcher();
        this.finderUtils = new finderutils_1.FinderUtils(extension);
        this.pathUtils = new pathutils_1.PathUtils(extension);
        this.registerSetEnvVar();
    }
    getCachedContent(filePath) {
        return this.cachedContent[filePath];
    }
    setCachedContent(filePath, cacheEntry) {
    ...
    findWorkspace() {
        var _a;
        const firstDir = (_a = vscode.workspace.workspaceFolders) === null || _a === void 0 ? void 0 : _a[0];
    ...
    async findRoot() {
        var _a;
        this.findWorkspace();
        const wsfolders = (_a = vscode.workspace.workspaceFolders) === null || _a === void 0 ? void 0 : _a.map(e => e.uri.toString(true));
     ...
     */
    async parseFileAndSubs(file, baseFile) {
     ...
     */
    getTeXChildren(file, baseFile, children, content) {
        ...
        if (this.cachedContent[file] === undefined) {
            ...
            while (true) {
                const result = pathRegexp.exec(content);
                if (!result) {
                    break;
                }
     ...
     */
    async parseInputFiles(content, currentFile, baseFile) {
        const pathRegexp = new pathutils_1.PathRegExp();
        while (true) {
            const result = pathRegexp.exec(content);
            if (!result) {
    ...
    async parseBibFiles(content, baseFile) {
    ...
    async parseAuxFile(content, srcDir) {
    ...
    async deleteFromFileWatcher(file) {
        const release = await this.fileWatcherMutex.acquire();
        try {
    ...
    registerListeners(fileWatcher) {
        fileWatcher.on('add', (file) => this.onWatchingNewFile(file));
        fileWatcher.on('change', (file) => this.onWatchedFileChanged(file));
        fileWatcher.on('unlink', (file) => this.onWatchedFileDeleted(file));
    }
    async resetFileWatcher() {
        const release = await this.fileWatcherMutex.acquire();
        try {
    ...
    async onWatchedFileChanged(file) {
    ...
    async onWatchedFileDeleted(file) {
    ...
    buildOnFileChanged(file, bibChanged = false) {
        const configuration = vscode.workspace.getConfiguration('latex-workshop');
        if (configuration.get('latex.autoBuild.run') !== "onFileChange" /* onFileChange */) {
            return;
        }
    ...
    async updateCompleterOnChange(file) {
     ...
     */
    async updateCompleter(file, content) {
        var _a;
        this.extension.completer.citation.update(file, content);
        const languageId = (_a = vscode.window.activeTextEditor) === null || _a === void 0 ? void 0 : _a.document.languageId;
